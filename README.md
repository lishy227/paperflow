...
> 🆓 **没账号也能体验**：内置 OpenAlex 免费源（纯 API，无需 key、无需浏览器、无需会话）。
> 新建任务默认就是免费源，5 分钟跑通全管道；有数据库访问账号的人勾选扩展数据库即可升级。
...
# 3. 启动浏览器 + 各库认证（首次需要，会话持久化在 .browser-profile/）
...
#    在弹出的浏览器里认证 IEEE / Scopus / ACM / EV（数据库访问账号，SSO 过期需重登）

# 4. 打开 Web 控制台（推荐）或直接用 CLI
python webui.py            # 自动打开 http://127.0.0.1:8080
#   CLI 替代：
#   python main.py init    # 问答式创建任务（选扩展数据库编号）
...
- **新建任务**：表单（标题 / 关键词 / 同义词 / 子方向 / 排除 / 年份 / **数据库分组选择** / 目标篇数）
  - 数据库分两组：**免费源（OpenAlex，无需账号，默认勾选）** + **扩展数据库（需会话）**
...
| `python launch_browser.py --kill` | 关闭本脚本启动的浏览器（保留会话） |
...
├─ step1_search/           # 各库搜索结果（扩展数据库 + 免费源 openalex_results.json）
...
- **健康检查每次 run 都真实执行**（不跳过）：缺依赖 / 缺 key / 浏览器没启动 / 会话失效 → 直接报"为什么失败 + 你该做什么"
...
launch_browser.py        浏览器启动/复用（独立 profile 持久化会话，--open 可多 URL，同站点复用标签页）
...
scripts/health_check.py  统一健康检查（依赖/key/浏览器/会话，CLI + Web 共用；免费源任务自动跳过 key/浏览器/会话）
...
| | 免费源（OpenAlex） | 扩展数据库（IEEE / Scopus / ACM / EV） |
|---|---|---|
| 需要 | 无（纯 API） | 访问会话 + SCOPUS_API_KEY |
...
- 免费源结果同样写缓存（有 DOI 走 `10.xxx` 共享缓存，无 DOI 走 `arxiv_xxx` / `openalex_Wxxx` 主键缓存），
  之后跑扩展数据库任务可直接命中
- 健康检查按任务选择自动降级：全免费源任务跳过 key/浏览器/会话检查

## 已知注意事项

- **免费源覆盖偏科**：OpenAlex 对 CS、EE、物理覆盖好，偏门工程领域结果可能偏少——适合体验管道和快速预览，正式综述建议加扩展数据库
- **会话会过期**：ACM 是 SSO 统一认证，会话过期会自动跳认证页；IEEE 数据库访问同样可能过期。健康检查会准确报出哪个库失效，去浏览器重登即可（免费源任务不受影响）
- **ACM 偶发 Cloudflare 人机验证**：健康检查偶报，重试或手动过验证即好
- **浏览器/CDP**：搜索阶段需要浏览器在线（端口 18800，`.browser-profile/` 持久化会话），先跑 `open-databases.bat` 或 `launch_browser.py`
